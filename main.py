...
async def get_memberlist(db: AsyncSession):
    try:
        query = text(
            "SELECT lm.*, lcc.classTitle, lr.rankTitlekor FROM chyMember lm left join chyClass lcc on lm.classNo = lcc.classNo "
...
async def get_rankmemberlist(rankno: int, db: AsyncSession):
    try:
        query = text("SELECT * FROM chyMember where rankNo = :rankno")
...
async def get_classstaffwithname(classno: int, db: AsyncSession):
    try:
        query = text(
            ...
            "FROM chyClassstaff s "
            "LEFT JOIN chyMember pm ON s.presidentNo = pm.memberNo "
            "LEFT JOIN chyMember sm ON s.secretNo = sm.memberNo "
            "LEFT JOIN chyMember tm ON s.trNo = tm.memberNo "
            "LEFT JOIN chyMember ltm ON s.ltNo = ltm.memberNo "
            "LEFT JOIN chyMember ttm ON s.ttNo = ttm.memberNo "
            "LEFT JOIN chyMember prm ON s.prpresidentNo = prm.memberNo "
...
async def get_ranklist(db: AsyncSession):
    try:
        query = text("SELECT * FROM chyRank where attrib not like :attpatt order by orderNo")
        result = await db.execute(query, {"attpatt": "%XXX%"})
...
async def get_boarddtl(boardno: int, db: AsyncSession):
    try:
        query = text("SELECT * FROM chyBoard where boardNo = :boardno")
...
async def get_requests(db: AsyncSession):
    try:
        query = text(
            "SELECT a.*, b.memberName FROM requestMessage a left join chyMember b on a.memberNo = b.memberNo where a.attrib not like :attxxx")
...
@app.post("/uploaddoc/{classno}")
async def upload_doc(request: Request, classno: int, file: UploadFile = File(...),
...
async def login(
    ...
    query = text(
        "SELECT userNo, userName,userRole, defaultClass FROM chyUser WHERE userId = :username AND userPassword = password(:password)")
    ...
    request.session["user_Class"] = user[3]
    return RedirectResponse(url="/success", status_code=303)
...
async def success_page(request: Request):
    ...
    return templates.TemplateResponse("member/mainClass.html",
                                      {"request": request, "user_No": user_No, "user_Name": user_Name,
                                       "user_Role": user_Role, "user_Class": user_Class})
...
async def memberList(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    return templates.TemplateResponse("admin/memberList.html",
                                      {"request": request, "user_No": user_No, "user_Name": user_Name,
                                       "members": members, "user_Class": user_Class})
...
async def addmember(request: Request, db: AsyncSession = Depends(get_db)):
    memberName = "신규추가 회원"
    query = text(f"INSERT into chyMember (memberName) values (:membername)")
...
async def memberList(request: Request, memberno: int, db: AsyncSession = Depends(get_db)):
    ...
    classlist = await get_classlist(db)
    ...
    return templates.TemplateResponse("member/memberDetail.html",
                                      {"request": request, "user_No": user_No, "user_Name": user_Name,
                                       ...
                                       "ranklist": ranklist, "ncphoto": ncphoto, "user_Class": user_Class})
...
async def update_memberdtl(request: Request, memberno: int, db: AsyncSession = Depends(get_db)):
    ...
    data4update = {
        ...
        "classNo": form_data.get("classlst"),
    ...
    query = text(f"UPDATE chyMember SET {set_clause} WHERE memberNo = :memberNo")
...
async def classList(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    user_Name = request.session.get("user_Name")
    user_Class = request.session.get("user_Class")
    class_list = await get_classlist(db)
    if not user_No:
        return RedirectResponse(url="/")
    return templates.TemplateResponse("member/classList.html",
                                      {"request": request, "user_No": user_No, "user_Name": user_Name,
                                       "class_list": class_list, "user_Class": user_Class})
...
async def update_rankdtl(request: Request, rankno: int, db: AsyncSession = Depends(get_db)):
    ...
    query = text(
        f"UPDATE chyRank SET rankTitlekor = :rankTitlekor, rankTitleeng = :rankTitleeng, rankDiv = :rankDiv, orderNo = :orderNo, useYN = :useYN WHERE rankNo = :rankNo")
...
async def addboard(request: Request, classno: int, db: AsyncSession = Depends(get_db)):
    form_data = await request.form()
...
async def slogan_image(classno: int, db: AsyncSession = Depends(get_db)):
    ...
    save_path = os.path.join(save_dir, f"{classno}logo.png")
...
async def docviewer(docno: int, db: AsyncSession = Depends(get_db)):
    try:
        query = text(
            "SELECT cDocument, docTitle from chyDoc where docNo = :docno ")
        result = await db.execute(query, {"docno": docno})
        row = result.fetchone()
...
async def mlogin(phoneno: str, db: AsyncSession = Depends(get_db)):
    try:
        ...
        result = {"classno": rows[0], "memberno": rows[1]}
        print(result)
    except:
        print("mLogin error")
    finally:
...
async def mskyn(memberno:int,msk:str, db: AsyncSession = Depends(get_db)):
    try:
        query = text("UPDATE chyMember set maskYN = :msk where memberNo = :memberNo")
...
async def mskyn(memberno:int,db: AsyncSession = Depends(get_db)):
    try:
        query = text("select maskYN from chyMember where memberNo = :memberNo")
        result = await db.execute(query, {"memberNo": memberno })
        rows = result.fetchone()
